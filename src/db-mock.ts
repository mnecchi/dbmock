...
const getObject = async <T>(path: string): Promise<{ [x: string]: T } & { id: string }> =>
  JSON.parse(readFileSync(path, 'utf-8'));

// function that writes to a "table" file
const putObject = (jsonFormatter: (o: any) => string) => (path: string, data: any) =>
